...
as $$
declare
  v_state jsonb; v_out jsonb := '[]'::jsonb; v_campaign jsonb; v_changed boolean := false;
  v_now timestamptz := now(); v_today date := (now() at time zone 'America/Sao_Paulo')::date;
  v_end date; v_before text; v_closed jsonb := '[]'::jsonb;
  v_service uuid := 'a02201ec-96d3-433f-b779-b81a4ee10066'::uuid; v_end_raw text; v_status_norm text;
begin
  select valor into v_state from public.operacional_estado where chave='central.campaigns.vitor-gutierrez' and dono is null for update;
  ...
  loop
    ...
    v_status_norm := translate(lower(v_before),'çãáàâäéèêëíìîïóòôöõúùûü','caaaaeeeeiiiiooooouuuu');
    if nullif(v_campaign->>'archivedAt','') is null and v_end is not null and v_end < v_today and v_status_norm='em execucao' then
