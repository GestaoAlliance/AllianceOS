...
declare
  ...
  v_customers_month bigint := 0;
  v_new_customers_month bigint := 0;
  v_returning_customers_month bigint := 0;
  v_gross_month numeric := 0;
  v_refunds_month numeric := 0;
  v_net_month numeric := 0;
  v_ticket_month numeric := 0;
  v_orders_today bigint := 0;
  v_customers_today bigint := 0;
...
begin
  ...
  select
    ...
    count(distinct nullif(cliente_hash,'')),
  ...
  select
    ...
    count(distinct nullif(cliente_hash,'')),
    coalesce(sum(greatest(coalesce(total,0) - coalesce(refunded_amount,0),0)),0)
  into v_orders_today, v_customers_today, v_net_today
  from alliance_data.commerce_orders
  where brand_id = p_brand_id
  ...
  if v_orders_month = 0
    ...
    v_customers_month := coalesce((v_snapshot->>'customers_month')::bigint,0);
    v_new_customers_month := coalesce((v_snapshot->>'new_customers_month')::bigint,0);
    v_returning_customers_month := coalesce((v_snapshot->>'returning_customers_month')::bigint,0);
    ...
    if coalesce(v_snapshot->>'date','') = to_char(v_today,'YYYY-MM-DD') then
      ...
      v_customers_today := coalesce((v_snapshot->>'customers_today')::bigint,0);
  ...
  return jsonb_build_object(
    ...
    'sales', jsonb_build_object(
      ...
      'customers_month', coalesce(v_customers_month,0),
      'new_customers_month', coalesce(v_new_customers_month,0),
      'returning_customers_month', coalesce(v_returning_customers_month,0),
      ...
      'customers_today', coalesce(v_customers_today,0),
    ...
    'shopify', jsonb_build_object(
      ...
      'has_customer_data', coalesce(v_customers_month,0) > 0,
      'data_mode', v_data_mode,
