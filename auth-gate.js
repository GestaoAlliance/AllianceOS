(function authGateClient(){
  ...
  (async()=>{try{const cfgRes=await nativeFetch(CONFIG_URL,{cache:'no-store'});if(!cfgRes.ok)throw new Error('Não foi possível carregar a configuração do AllianceOS.');const cfg=await cfgRes.json();const mod=await import('https://esm.sh/@supabase/supabase-js@2.116.0');client=mod.createClient(cfg.url,cfg.anon,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});const result=await client.auth.getSession();await acceptSession(result.data.session);client.auth.onAuthStateChange((event,next)=>{if(event==='SIGNED_OUT'){clearWorkspaceCache();location.replace('/login');return}if(event==='PASSWORD_RECOVERY'){session=next;renderRecovery();return}session=next})}catch(err){showRoot();root().innerHTML='<div class="auth-fatal"><strong>Não foi possível iniciar o AllianceOS</strong><span>'+esc(err&&err.message?err.message:String(err))+'</span><button onclick="location.reload()">Tentar novamente</button></div>';finish({authenticated:false,error:String(err),client:null,session:null,context:null})}})();
})();