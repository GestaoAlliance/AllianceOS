(() => {
  ...
  const enhance = (root) => {
    ...
    if (quote) quote.innerHTML = '“Operação, pessoas e dados. Tudo no mesmo fluxo.”';
    const avatar = root.querySelector('.auth-person-avatar');
    if (avatar && !avatar.querySelector('img')) {
      avatar.innerHTML = `<img src="${PROFILE_PHOTO}" alt="Foto de Vitor Gutierrez">`;
    }
    const name = root.querySelector('.auth-person-name');
    const role = root.querySelector('.auth-person-role');
    if (name) name.textContent = 'Vitor Gutierrez';
    if (role) role.textContent = 'Gestor de Operações';
  ...
  const observer = new MutationObserver(() => enhance(document.getElementById('alliance-auth-root')));
